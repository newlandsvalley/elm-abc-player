...
type Msg
    ...
    | Play
    | PlayStarted Bool
      -- response from the player that it's started
    | PlayCompleted
      -- the play has completed (we compute the time ourselves)
    | ShowButtons
      -- immediately after play has ended
    | Move Bool
    | MoveToEnd Bool
    | Error ParseError
...
update msg model =
    case msg of
        ...
        Abc s ->
            ( { model | abc = s }, Cmd.none )

        Play ->
            playAbc model

        PlayStarted _ ->
            ( model, (suspend model.duration) )
        ...
        Move b ->
            let
                next =
                    case b of
                        True ->
                            Basics.min (model.lessonIndex + 1) (Array.length lessons - 1)

                        False ->
                            Basics.max (model.lessonIndex - 1) 0
            in
                ( { model
                    | lessonIndex = next
                    , abc = (example next)
        ...
        Error pe ->
            ( { model | error = Just pe }, showButtonsAction )
...
suspend secs =
    let
        _ =
            log "suspend time" secs

        time =
            secs * 1000
    in
        Process.sleep time
            |> Task.perform (\_ -> PlayCompleted)
...
showButtonsAction =
    Task.perform (\_ -> NoOp) showButtons
        ...
        case abcTuneResult of
            Ok _ ->
                ...
                in
                    ...
                    , requestPlayNoteSequence (List.reverse notesReversed)
                    )
    ...
    if (model.fontsLoaded) then
        div []
            ...
            , div []
                ...
                , img
                    [ src (scoreUrl model.lessonIndex)
                    , rightImageStyle
                    ]
                    []
                ]
...
title i =
    ...
    in
        case mlesson of
            ...
            Just l ->
                "ABC Tutorial: lesson " ++ (toString (i + 1) ++ " - " ++ l.title)
...
scoreUrl i =
    ...
    in
        case mlesson of
            ...
            Just l ->
                "assets/images/tutorial/" ++ l.id ++ ".png"
