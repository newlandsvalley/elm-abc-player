...
moveBodyPart i bp =
  case bp of
    Score ms -> Score (moveMusicList i ms)
    _ -> bp

moveOctave : Int -> Music -> Music
moveOctave i m =
  case m of
    Note n -> Note (moveNoteBy i n)

    BrokenRhythmPair n1 b n2 ->  BrokenRhythmPair (moveNoteBy i n1) b (moveNoteBy i n2)

    Tuplet ts ns -> Tuplet ts (moveNoteList i ns)

    GraceNote b m -> GraceNote b (moveOctave i m)

    Chord c -> Chord (moveChord i c)

    NoteSequence ms -> NoteSequence  (moveMusicList i ms)
